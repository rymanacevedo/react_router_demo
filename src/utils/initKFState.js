function setupKFState() {
   window.KF = window.KF || {
      state: {
         ...
         baseUri: window.location.origin === "http://localhost:3000" ? "http://mybob.amplifire.me:8080" : window.location.origin,
         // TODO need to get from server for when we replace LearnerDashboardEntry.java
         amp7LearningUrl: window.location.origin === "http://localhost:3000" ? "http://mybob.amplifire.me:8080" : window.location.origin,
      },
   };
}
